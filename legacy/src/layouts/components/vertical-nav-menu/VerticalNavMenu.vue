    ...
    <vs-sidebar
      ...
      <div @mouseenter="mouseEnter" @mouseleave="mouseLeave">
        ...
        <div slot="header" class="header-sidebar flex items-end justify-between">
          ...
          <!-- Logo -->
          <router-link class="vx-logo cursor-pointer flex items-center" tag="div" to="/">
            <logo class="w-10 mr-4 fill-current text-primary"/>
            <span v-if="title" v-show="isMouseEnter || !reduce" class="vx-logo-text text-primary">{{ title }}</span>
          </router-link>
          <!-- /Logo -->
          ...
          <div>
            ...
            <template v-if="showCloseButton">
              <feather-icon class="m-0 cursor-pointer" icon="XIcon"
        ...
        <!-- Menu Items -->
        <component :is="scrollbarTag" :key="$vs.rtl" ref="verticalNavMenuPs" :settings="settings"
                   class="scroll-area-v-nav-menu pt-2" @scroll="psSectionScroll" @ps-scroll-y="psSectionScroll">
          <template v-for="(item, index) in menuItemsUpdated">

            <!-- Group Header -->
            <span v-if="item.header && !verticalNavMenuItemsMin" :key="`header-${index}`"
            ...
            <template v-else-if="!item.header">
              ...
              <v-nav-menu-item
                ...
                <vs-chip v-if="item.tag && (isMouseEnter || !reduce)" :color="item.tagColor" class="ml-auto">{{
                    item.tag
                  }}
                </vs-chip>
              </v-nav-menu-item>
