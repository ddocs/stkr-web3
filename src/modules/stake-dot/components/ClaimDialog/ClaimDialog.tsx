...
interface IClaimDialogProps {
  ...
  onClose?: () => void;
  onSubmit: (values: IClaimFormValues) => void;
}
...
export const ClaimDialog = ({
  ...
  const setMaxAmount = useCallback(
    (form: FormApi<IClaimFormValues>, maxAmount: string) => () =>
      form.change('amount', maxAmount),
    [],
  );

  const validate = useCallback(
    ({ amount, address }: IClaimFormValues) => {
      const currentAmount = new BigNumber(amount);
      const minAmount = new BigNumber(MIN_AMOUNT);
      const errors: FormErrors<IClaimFormValues> = {};
      const isAddress =
    ...
    >
      <Container className={classes.container}>
        <Typography variant="h3" className={classes.title}>
          {t('stake-dot.claim-dialog.title')}
        </Typography>
            ...
            >
              <Box mb={4}>
                {(withBalance || isBalanceLoading) && (
                  ...
                  >
                    ...
                    {isBalanceLoading ? (
                      ...
                      `${roundedBalance} ${t('stake-dot.aDotb')}`
                    )}
                  </Typography>
                )}
                ...
                <Field
                  ...
                  name="amount"
                  label={t('stake-dot.claim-dialog.amount')}
                  placeholder="0"
                  variant="outlined"
              ...
              <Box mb={4}>
                <Field
                  ...
                  component={InputField}
                  name="address"
                  label={t('stake-dot.claim-dialog.address')}
                  variant="outlined"
                  disabled={submitDisabled}
                  fullWidth
                />
          ...
          {isLoading ? (
            ...
            <div className={classes.btnWrapper}>
              <Button
                ...
                isLoading={isLoading}
              >
                {t('stake-dot.claim-dialog.submit')}
              </Button>
            </div>
          )}
        </Container>
