  ...
  <vx-card class="overflow-hidden">
    <div slot="no-body">
      <div :class="{
        ...
        <feather-icon :class="[`text-${color}`, {'mb-4': !iconRight}]" :icon="icon" :style="{background: `rgba(var(--vs-${color}),.15)`}"
                      class="p-3 inline-flex rounded-full"></feather-icon>
        <div class="truncate">
      ...
      <div v-if="!hideChart" class="line-area-chart">
        <vue-apex-charts ref="apexChart" :options="chartOptions" :series="chartData" :type="type" height="100"
                         width="100%"/>
      </div>
    </div>
  </vx-card>
