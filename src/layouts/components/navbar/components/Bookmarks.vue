  ...
  <div class="navbar-bookmarks flex items-center">
    ...
    <ul class="vx-navbar__starred-pages">
      <draggable v-model="starredPagesLimited" :group="{name: 'pinList'}" class="flex cursor-move">
        <li v-for="page in starredPagesLimited" :key="page.url" class="starred-page">
          <vx-tooltip :text="page.title" delay=".3s" position="bottom">
            <feather-icon :icon="page.icon" :svgClasses="['h-6 w-6 stroke-current', textColor]"
                          class="p-2 cursor-pointer" @click="$router.push(page.url).catch(() => {})"/>
          </vx-tooltip>
        </li>
      </draggable>
    </ul>
    ...
    <div v-if="starredPagesMore.length" class="vx-navbar__starred-pages--more-dropdown">
      <vs-dropdown class="cursor-pointer" vs-custom-content vs-trigger-click>
        <feather-icon class="cursor-pointer p-2" icon="ChevronDownIcon" svgClasses="h-4 w-4"></feather-icon>
        <vs-dropdown-menu>
          <ul class="vx-navbar__starred-pages-more--list">
            <draggable v-model="starredPagesMore" :group="{name: 'pinList'}" class="cursor-move">
              <li v-for="page in starredPagesMore" :key="page.url"
                  class="starred-page--more flex items-center cursor-pointer"
                  @click="$router.push(page.url).catch(() => {})">
                <feather-icon :icon="page.icon" :svgClasses="['h-5 w-5 stroke-current', textColor]"
                              class="ml-2 mr-1"></feather-icon>
    ...
    <div class="bookmark-container">
      <feather-icon :svgClasses="['stoke-current text-warning', textColor]" class="cursor-pointer p-2" icon="StarIcon"
                    @click.stop="showBookmarkPagesDropdown = !showBookmarkPagesDropdown"/>
      <div v-if="showBookmarkPagesDropdown" v-click-outside="outside"
